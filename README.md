...
**Mission-Control-Token (einmalig):** Für GitHub-Live-Daten und die Demo-Steuerung (Szenarien starten, Freigaben erteilen) braucht die App einen **fine-grained PAT**, der nur für dieses Repo gilt: GitHub → Settings → Developer settings → [Fine-grained tokens](https://github.com/settings/personal-access-tokens/new) → Repository access: *Only select repositories* → `NextGen-CICD` → Permissions: **Actions (Read and write)**, **Deployments (Read and write)**, **Contents (Read-only)**. Dann sicher im **macOS-Keychain** speichern (verschlüsselt at rest, keine Klartext-Datei):

```bash
deploy/scripts/stacks.sh token     # fragt den Token ohne Echo ab und legt ihn in den Keychain
```

`stacks.sh up` liest den Keychain-Eintrag automatisch (Fallback: `~/deployments/mission-control.env`, wird beim Keychain-Setup aufgeräumt); ohne Token läuft Mission Control als reine Umgebungs-Visualisierung (GitHub-Teil aus). Die App ist bewusst nur an `127.0.0.1` gebunden; im laufenden Container ist der Token prinzipbedingt per `docker inspect` sichtbar — auf einem Single-User-Demo-Mac akzeptabel.
