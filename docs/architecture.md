...
Token-Handling: Der fine-grained PAT (nur dieses Repo, Actions/Deployments R+W, Ablaufdatum) liegt im **macOS-Keychain** (`stacks.sh token`) — dieselbe Ablage, die auch die gh-CLI für ihren eigenen Token nutzt; `stacks.sh up` liest ihn in der interaktiven Session und injiziert ihn in den Container. **Ausbaustufe für den Enterprise-Einsatz:** zentrales Secret-Management (HashiCorp Vault/OpenBao) mit kurzlebigen, automatisch rotierten Tokens und Audit-Log — für einen Einzel-Demo-Rechner bewusst nicht nötig.
